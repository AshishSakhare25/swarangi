    ...
    <section data-testid="process-section" className="border-t border-line py-28 sm:py-36">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-16 flex flex-col gap-16 lg:flex-row lg:items-start lg:gap-12">
            ...
            <div className="flex flex-col gap-10 md:flex-row md:items-start md:gap-0">
              {STEPS.map((s, i) => (
                <Reveal key={s.id} delay={0.1 * i} className="flex-1">
                  <div data-testid={`process-step-${s.id}`} className="relative flex items-center gap-5 md:flex-col md:items-start md:gap-0 md:pr-6">
                    ...
                    <p className="mt-0 hidden font-mono text-[11px] uppercase tracking-[0.18em] text-ink md:mt-5 md:block">{s.label}</p>
                    <p className="font-hand text-xl text-ember md:mt-2" style={{ transform: `rotate(${i % 2 === 0 ? -2 : 2}deg)` }}>
                      {s.note}
                    </p>
                    {i < STEPS.length - 1 && (
                      <HandArrow className="absolute -right-3 top-8 hidden w-12 opacity-50 md:block" />
                    )}
                  </div>
                </Reveal>
              ))}
          ...
          <div className="w-full shrink-0 pt-8 lg:w-[340px] lg:pt-14">
            <Recipe />
          </div>
        </div>
      </div>
